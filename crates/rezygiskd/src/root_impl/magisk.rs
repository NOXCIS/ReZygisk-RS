...
pub fn magisk_uid_is_manager(uid: u32) -> bool {
    ...
    if !output.is_empty() {
        // C: output + strlen("value=") — sqlite prints "value=<name>"
        if let Some(requester) = output.strip_prefix("value=") {
            stat_path = format!("/data/user_de/0/{}", requester.trim());
        } else {
            stat_path = format!("/data/user_de/0/{}", output.trim());
        }
    }
