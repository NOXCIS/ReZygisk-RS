...
# Truman Phase 7 — built-in reflection-spoof sub-module. zygiskd fd-loads it
# from its OWN module dir (/data/adb/modules/rezygisk/zygisk/truman/<abi>.so)
# ahead of every third-party module. Fail-soft: a zip built without a target
# arch's lib just logs and continues.
ui_print "- Extracting built-in truman sub-module"
mkdir -p "$MODPATH/zygisk/truman"
for truman_abi in arm64-v8a armeabi-v7a; do
  if unzip -l "$ZIPFILE" "zygisk/truman/$truman_abi.so" 2>/dev/null | grep -q "zygisk/truman/$truman_abi.so"; then
    extract "$ZIPFILE" "zygisk/truman/$truman_abi.so" "$MODPATH/zygisk/truman" true
  else
    ui_print "! zygisk/truman/$truman_abi.so missing from zip (skipping)"
  fi
done
